 ...
 *
 * Этот скрипт загружает таблицы и блюда, позволяет добавлять новые позиции в заказ, 
 ...
 *
 * Основные действия:
 * 1. Загрузка таблиц (`loadTables`) и блюд (`loadDishes`), которые отображаются в селекторах.
...
const tablesApiUrl = '/api/v1/tables/' // URL для получения таблиц
const dishesApiUrl = '/api/v1/dishes/' // URL для получения блюд
const createOrderApiUrl = '/api/v1/orders/' // URL для создания заказа

/**
 * Загружает данные о таблицах, блюдах и выручке за смену при загрузке страницы.
 */
document.addEventListener('DOMContentLoaded', async function () {
    await loadTables() // Загружаем таблицы
    await loadDishes() // Загружаем блюда
    await loadShiftRevenue() // Загружаем выручку за смену
})

/**
 * Загружает список доступных таблиц с сервера и заполняет элемент выбора таблицы.
 */
